...
import com.eynnzerr.bandoristation.model.account.UpdateBannerResult
...
class UpdateBannerUseCase(
    ...
    override suspend fun execute(parameters: String): UseCaseResult<String, String> {
        ...
        repository.sendAuthenticHttpsRequest(
            ...
            onSuccess = { response ->
                val bannerName = NetResponseHelper.parseApiResponse<UpdateBannerResult>(response)
                return bannerName?.let { UseCaseResult.Success(it.banner) } ?: UseCaseResult.Error("Failed to parse UpdateBanner response.")
            },
            onFailure = {
